...
export default class ChangedFileContainer extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      ignoreEolWhitespace: this.props.config.get('github.ignoreEolWhitespace'),
      ignoreWhitespaceChange: this.props.config.get('github.ignoreWhitespaceChange'),
      ignoreWhitespace: this.props.config.get('github.ignoreWhitespace'),
    };

    this.subs = new CompositeDisposable();
    this.subs.add(
      this.props.config.onDidChange('github.ignoreEolWhitespace', this.updateIgnoreEolWhitespace),
      this.props.config.onDidChange('github.ignoreWhitespaceChange', this.updateIgnoreWhitespaceChange),
      this.props.config.onDidChange('github.ignoreWhitespace', this.updateIgnoreWhitespace),
    );
  }
  ...
  fetchData(repository) {
    ...
    return yubikiri({
      multiFilePatch: repository.getFilePatchForPath(this.props.relPath, {
        ...
        before,
        after,
        ignoreEolWhitespace: this.state.ignoreEolWhitespace,
        ignoreWhitespaceChange: this.state.ignoreWhitespaceChange,
  ...
  updateIgnoreEolWhitespace = ({newValue}) => {
    if (newValue !== this.state.ignoreEolWhitespace) {
      this.setState({ignoreEolWhitespace: newValue});
    }
  }

  updateIgnoreWhitespaceChange = ({newValue}) => {
    if (newValue !== this.state.ignoreWhitespaceChange) {
      this.setState({ignoreWhitespaceChange: newValue});
    }
  }

  updateIgnoreWhitespace = ({newValue}) => {
    if (newValue !== this.state.ignoreWhitespace) {
      this.setState({ignoreWhitespace: newValue});
    }
  }
}
