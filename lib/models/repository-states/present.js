...
export default class Present extends State {
  ...
  getFilePatchForPath(filePath, options) {
    ...
    const conf = {
      staged: opts.staged,
      ignoreEolWhitespace: opts.ignoreEolWhitespace,
      ignoreWhitespaceChange: opts.ignoreWhitespaceChange,
      ignoreWhitespace: opts.ignoreWhitespace
    };
    return this.cache.getOrSet(Keys.filePatch.oneWith(filePath, conf), async () => {
      const diffs = await this.git().getDiffsForFilePath(filePath, conf);
...
const Keys = {
  ...
  filePatch: {
    _optKey: ({staged, ignoreEolWhitespace, ignoreWhitespaceChange, ignoreWhitespace}) => {
      return ((staged ? 's': 'u') + (ignoreWhitespace ? 'w' : (ignoreWhitespaceChange ? 'c' : ignoreEolWhitespace ? 'e' : 'a')))
    },
